...
namespace DEFRA.NE.BNG.Integration.Infrastructure.UnitTests
{
    public abstract class TestBase<T>
    {
        ...
        public static DefraIdAccount CreateTestDefraIdAccount()
        {
            var account = new DefraIdAccount()
            {
                ...
                defra_dateofincorporation = DateTime.Now.ToString("dddd, dd MMMM yyyy"),
        ...
        protected List<Contact> GenerateSampleContacts(int entityCount)
        {
            var entities = new List<Contact>();

            for (int i = 0; i < entityCount; i++)
            {
                var item = new Contact() { Id = Guid.NewGuid() };
                item[DataverseExtensions.AttributeLogicalName<Contact>(nameof(Contact.AssistantName))] = Guid.NewGuid().ToString();
                item[DataverseExtensions.AttributeLogicalName<Contact>(nameof(Contact.EMailAddress1))] = $"john{i}.smith{i}@bng_test.com";
                item[DataverseExtensions.AttributeLogicalName<Contact>(nameof(Contact.AccountId))] = new EntityReference(Account.EntityLogicalName, Guid.NewGuid());
                item[DataverseExtensions.AttributeLogicalName<Contact>(nameof(Contact.LastName))] = $"Smith{i}";
                item[DataverseExtensions.AttributeLogicalName<Contact>(nameof(Contact.FirstName))] = $"John{i}";
        ...
        protected static async Task<U> HydrateFromTestDataFile<U>(string filename)
        {
            var data = await System.IO.File.ReadAllTextAsync(filename);

            var result = JsonConvert.DeserializeObject<U>(data);
            return result;
        }
    }
}